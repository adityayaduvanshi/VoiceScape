...
function ControlTray({trayRef}: ControlTrayProps) {
  ...
  const { setCameraTarget, setMarkers, setUserLocation, clearMarkers } = useMapStore();
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                clearMarkers();
                setMarkers([{
                    position: { ...userCoords, altitude: 1 },
                    label: 'You are here',
                    showLabel: true,
                }]);

                setCameraTarget({
                    center: { ...userCoords, altitude: 200 },
                    range: 1000,
                    tilt: 45,
                    heading: 0,
                    roll: 0,
                ...
                useLogStore.getState().addTurn({
                    role: 'system',
                    text: `Location set to your current position. You can now ask for things "near me" or "around here".`,
                    isFinal: true,
                });
            },
            (error) => {
                ...
                useLogStore.getState().addTurn({
                    role: 'system',
                    text: `Could not get your location. Error: ${error.message}`,
                    isFinal: true,
                });
            },
